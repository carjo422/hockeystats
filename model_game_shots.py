...
def update_shots_model_linreg(seasonYear,c):

    #Model for home shots

    c.execute("SELECT GAMEID, AHS, AASA, SCORE1, SCORE2, ACT_SHOTS1 FROM EXP_SHOTS_TABLE WHERE SEASON < ?",[seasonYear])
    ...
    filename = data_directory + '/models/lm_home_shots_' + str(seasonYear) + '.sav'

    pickle.dump(lm_home_shots, open(filename, 'wb'))

    # Model for away shots

    c.execute("SELECT GAMEID, AAS, AHSA, SCORE1, SCORE2, ACT_SHOTS2 FROM EXP_SHOTS_TABLE WHERE SEASON < ?",[seasonYear])
    regdata = pd.DataFrame(c.fetchall())
    ...
    filename = data_directory + '/models/lm_away_shots_' + str(seasonYear) + '.sav'

    pickle.dump(lm_away_shots, open(filename, 'wb'))

    #Update model_shot_table with exp_shots

    c.execute("SELECT AHS, AASA, AAS, AHSA, SCORE1, SCORE2, GAMEID FROM EXP_SHOTS_TABLE WHERE SEASON = ?",[seasonYear])
    upd = c.fetchall()
...
def update_shots_model_forest(seasonYear,c):
    ...
    filename = data_directory + '/models/forest_home_shots_' + str(seasonYear) + '.sav'
    ...
    filename = data_directory + '/models/forest_away_shots_' + str(seasonYear) + '.sav'
...
def get_shots_goals_linreg(seasonYear, inputs, gameid, c):
...
def get_shots_goals_forest(seasonYear, inputs, gameid,c):
